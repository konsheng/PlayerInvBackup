...
public final class RestoreService {
    ...
    public void restoreExperienceToPlayer(CommandSender actor, Player target, String backupId) {
        ...
        Bukkit.getAsyncScheduler().runNow(plugin, ignored -> {
            ...
            if (!parts.hasExperienceData()) {
                runOnActor(actor, () -> Chat.error(actor, "errors.backup-experience-unavailable"));
                return;
            }
                ...
                try {
                    queued = backupService.requestBackup(target, TriggerType.MANUAL, (success, preRestoreBackupId) -> {
                        ...
                        runOnOnlineTarget(targetUuid, currentTarget -> {
                            ...
                            try {
                                applyExperienceSnapshot(currentTarget, parts);
                            } catch (Exception e) {
                                plugin.getLogger().log(
                                        Level.SEVERE,
                                        plugin.lang().plain(
                    ...
                    runOnActor(actor, () -> Chat.error(actor, "errors.restore-experience-pre-backup-failed"));
                    return;
                }
    ...
    private static void applyExperienceSnapshot(Player target, SnapshotParts parts) {
        target.closeInventory();
        target.setExp(0.0f);
        target.setLevel(0);
